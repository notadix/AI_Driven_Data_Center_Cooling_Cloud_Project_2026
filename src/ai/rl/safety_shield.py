...
class SafetyShield:
    def __init__(self, constants: CoolingConstants = None,
        ...
        # Only used for the opt-in flow-coupled plant (DataCenterCoolingEnv(flow_coupling=...)); 0 keeps
        # the original model in which the pump does not influence rack inlet temperature.
        self.coupling = 0.0

    # -- prediction ------------------------------------------------------
    def predict_inlet(self, supply_c: float, ambient_c: float, a0, a3, it_kw: float = None, a1=None):
        ...
        With a flow-coupled plant, `it_kw` and the pump action `a1` are needed as well."""
        c = self.c
        ...
        if self.coupling and it_kw is not None and a1 is not None:
            pump_pct = 35.0 + (np.asarray(a1) + 1.0) * 0.5 * 65.0
            flow = c.FLOW_MIN_LPM + pump_pct / 100.0 * c.FLOW_SPAN_LPM
            mass = flow / 60.0 * c.DENSITY_KG_L + 1e-6
            base_inlet = base_inlet + self.coupling * it_kw * c.HEAT_CAPTURE / (mass * c.CP_KJ_KGK)
    ...
    def filter(self, obs: np.ndarray, action: np.ndarray) -> Tuple[np.ndarray, float]:
        ...
        if self.is_safe(obs, action):
            return action, 0.0
        if self.coupling:
            return self._filter_coupled(obs, action)
    ...
    def _filter_coupled(self, obs: np.ndarray, action: np.ndarray) -> Tuple[np.ndarray, float]:
        """Closest safe (supply, pump, valve) triple when pump speed also moves the inlet temperature."""
        supply_c, ambient_c, it_kw = float(obs[3]), float(obs[1]), float(obs[0])
        best = None
        for a1 in np.unique(np.concatenate([[action[1]], np.linspace(action[1], 1.0, 9)])):
            for a3 in np.unique(np.concatenate([[action[3]], np.linspace(-1.0, 1.0, 9)])):
                inlet = self.predict_inlet(supply_c, ambient_c, self._grid, a3, it_kw, a1)
                ok = (inlet >= self.lo) & (inlet <= self.hi)
                if not ok.any():
                    continue
                cand = self._grid[ok]
                a0 = cand[np.argmin(np.abs(cand - action[0]))]
                cost = abs(a0 - action[0]) + abs(a1 - action[1]) + abs(a3 - action[3])
                if best is None or cost < best[0]:
                    best = (cost, a0, a1, a3)
        safe = action.copy()
        if best is not None:
            safe[0], safe[1], safe[3] = best[1], best[2], best[3]
        else:  # emergency: most cooling available
            too_hot = float(self.predict_inlet(supply_c, ambient_c, action[0], action[3], it_kw, action[1])) > self.hi
            safe[0], safe[1], safe[3] = (-1.0, 1.0, -1.0) if too_hot else (1.0, action[1], action[3])
        return safe.astype(np.float32), float(np.abs(safe - action).sum())
